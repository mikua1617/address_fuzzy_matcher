...
def main():
        ...
        for i in numbers1:
            for j in numbers2:
                if(textdistance.levenshtein(i,j) == 0):
                    ...
                    l+=1
                    if not (i in similar_numbers.keys() and similar_numbers[i] == 1):
                        similar_numbers[i] = 0.5
    ...
    if not (len(keywords2) == 0 or len(keywords1) == 0):
        ...
        for i in keywords1:
            for j in keywords2:
                l+=1
                if (i not in similar_keywords.keys() or (similar_keywords[i] and similar_keywords[i] < textdistance.levenshtein.normalized_distance(i,j))):
                    similar_keywords[i] = 1 - textdistance.levenshtein.normalized_distance(i,j)
        if similar_keywords:
    ...
    total_score = 0.7*final_street_score + 0.3*score
